...
As an optional parameter, `plugin` parameter can be used to initialize the external Auth SPA Client.
...
export const MyApp = (): ReactElement => {
    return (
        <AuthProvider config={ config } plugin={ ChoreoSPAClient.getInstance() }>
            <Dashboard />
        </AuthProvider>
    )
}
